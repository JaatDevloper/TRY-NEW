...
async def receive_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if answer < 0 or answer >= len(context.user_data["options"]):
        await update.message.reply_text(
            f"Invalid answer. Please enter a number between 1 and {len(context.user_data['options'])}."
        )
        return ANSWER
    ...
    await update.message.reply_text(
        ...
        f"Correct answer: {question['options'][answer]}\n"
        f"Category: {category}\n\n"
    ...
    if poll_id == current_poll_id:
        ...
        if 0 <= current_index < len(questions):
            ...
            if str(user_id) not in participants:
                participants[str(user_id)] = {'correct': 0, 'answered': 0, 'name': poll_answer.user.first_name}
...
async def end_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Sort participants by score (correct answers)
...
async def handle_poll_answer_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for category in categories:
        keyboard.append([InlineKeyboardButton(category, callback_data=f"pollcat_{category}")])
    ...
    class MockContext:
        user_data = {
            'quiz': {
                ...
                'participants': {
                    '1234': {'name': 'Alice', 'correct': 2, 'answered': 2},
                    '5678': {'name': 'Bob', 'correct': 1, 'answered': 2},
                    '9012': {'name': 'Charlie', 'correct': 0, 'answered': 1},
                },
...
def main():
    ...
    # Add handler for custom ID input
